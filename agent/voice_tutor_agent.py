...
Since we STT can only handle one language at a time, you will be provided with two speech to text strings
...
class VoiceTutorAgent(BaseAgent):
    def __init__(self, 
        ...
        system_message = TUTOR_SYSTEM_MESSAGE.substitute(desc=tutor.description, lang1=inst_lang.name,
            lang2=subject.name, level_name=level.name, question=question_example, answer=answer_example)
